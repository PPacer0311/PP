...
import pickle
import random
from Model.Attention import TransMIL as Attention
from Model.Attention import TransMIL2
from Model.Attention import Attention_with_Classifier
from utils import get_cam_1d
import torch.nn.functional as F
from Model.network import DimReduction,Classifier_1fc
import numpy as np
...
parser.add_argument('--mDATA0_dir_train0', default='/scratch/PI/jhc/hlifv/DTFD/mDATA_train.pkl', type=str)  ## Train Set
parser.add_argument('--mDATA0_dir_val0', default='/scratch/PI/jhc/hlifv/DTFD/mDATA_train.pkl', type=str)      ## Validation Set
parser.add_argument('--mDATA_dir_test0', default='/scratch/PI/jhc/hlifv/DTFD/mDATA_test.pkl', type=str)         ## Test Set                
...
def main():
    ...
    classifier = Classifier_1fc(params.mDim, params.num_cls).to(params.device)
    Pclassifier = TransMIL2().to(params.device)
    attention = Attention().to(params.device)
    ...
    for ii in range(params.EPOCH):
        ...
        train_attention_preFeature_DTFD(classifier=classifier, dimReduction=dimReduction, attention=attention, UClassifier=attCls,Pclassifer=Pclassifier, mDATA_list=(SlideNames_train, FeatList_train, Label_train), ce_cri=ce_cri,
        ...
        auc_val = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,Pclassifer=Pclassifier,
        ...
        tauc = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,Pclassifer=Pclassifier,
...
def test_attention_DTFD_preFeat_MultipleMean(mDATA_list, classifier, dimReduction, attention, UClassifier,Pclassifer, epoch, criterion=None,  params=None, f_log=None, writer=None, numGroup=3, total_instance=3, distill='MaxMinS'):
            ...
            for tidx, tfeat in enumerate(batch_feat):
                ...
                for jj in range(params.num_MeanInference):
                    ...
                    for tindex in index_chunk_list:
                        ...
                        tattFeat_tensor = torch.sum(tAA, dim=0).unsqueeze(0)  ## 1 x fs

                        tPredict = classifier(tattFeat_tensor)  ### 1 x 2
                        slide_sub_preds.append(tPredict)

                        patch_pred_logits = get_cam_1d(classifier, tAA.unsqueeze(0)).squeeze(0)  ###  cls x n
...
def train_attention_preFeature_DTFD(mDATA_list, classifier, dimReduction, attention,UClassifier,Pclassifer,  optimizer0, optimizer1, epoch, ce_cri=None, params=None,
    ...
    for idx in range(numIter):
        ...
        for tidx, (tslide, slide_idx) in enumerate(zip(tslide_name, tidx_slide)):
            ...
            for tindex in index_chunk_list:
                ...
                tAA = attention(tmidFeat[None])
                
                
                #attFeats = torch.einsum('ns,n->ns', tmidFeat, tAA)  ### n x fs
                tattFeat_tensor = torch.sum(tAA, dim=0).unsqueeze(0)  ## 1 x fs
                tPredict = classifier(tattFeat_tensor)  ### 1 x 2
                slide_sub_preds.append(tPredict)
                ...
                if distill == 'MaxMinS':
                    slide_pseudo_feat.append(MaxMin_inst_feat.detach())
                elif distill == 'MaxS':
                    slide_pseudo_feat.append(max_inst_feat.detach())
                elif distill == 'AFS':
                    slide_pseudo_feat.append(af_inst_feat.detach())
 ...
 """
            ...
            gSlidePred = UClassifier(slide_pseudo_feat[None])['logits']
...
def reOrganize_mDATA_test(mDATA):
    ...
    tumorSlides = os.listdir('/scratch/PI/jhc/hlifv/DTFD/mask')
