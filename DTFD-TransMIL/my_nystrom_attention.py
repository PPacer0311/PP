...
class NystromAttention(nn.Module):
    ...
    #     print(self.pinv_iterations)
    #     print(self.eps)
        
    #     n, _, h, m, iters, eps = *x.shape, self.heads, self.num_landmarks, self.pinv_iterations, self.eps
    ...
    #     if exists(mask):
    #         mask = rearrange(mask, ' n -> () n')
    #         q, k, v = map(lambda t: t * mask[..., None], (q, k, v))
    #     q = q * torch.tensor(self.scale).clone().detach()
    #     # generate landmarks by sum reduction, and then calculate mean using the mask
    ...
    #     if self.residual:
    #         # out += self.res_conv(v)
    #         out += self.res_conv(v.unsqueeze(0)).squeeze(0)
